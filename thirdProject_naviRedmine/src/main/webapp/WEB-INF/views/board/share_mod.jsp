...
<form:form action="${pageContext.request.contextPath }/board/share/mod/${share.bpId }"
	id="updateForm" enctype="multipart/form-data" modelAttribute="share">
				...
				<strong class="view-tit d-flex align-items-center">
					...
					<div class=" col-sm-11">
						<input type="text" name="bpTitle" class="form-control"
							placeholder="나눔 할 물품을 입력해주세요!" value="${share.bpTitle}">
					</div>
				</strong>
				<ul class="ul clearfix">
					<li class="d-flex w-50  align-items-center mb-2"><span class="list-tit">작성자</span> <span>${share.empName }</span></li>
					<li class="d-flex w-50  align-items-center mb-2"><span class="list-tit">등록일자</span> <span>${share.bpRegDtStr }</span></li>
					<li class="d-flex w-50  align-items-center mb-2"><span class="list-tit">수정자</span> <span><c:if
								test="${empty share.bpModId}">수정내역 없음</c:if> <c:if
								test="${not empty share.bpModId}">ADMIN</c:if> </span></li>
					<li class="d-flex w-50  align-items-center mb-2"><span class="list-tit">수정일자</span> <span> <c:if
								test="${empty share.bpModDt }">수정내역 없음</c:if> <c:if
								test="${not empty share.bpModDt }">${share.bpModDtStr }</c:if>
					</span></li>
					<li class="d-flex w-50  align-items-center mb-2"><span class="list-tit">나눔상태</span> 
					<span>
					<select	class="form-select form-control" name="bpShrYn"
							aria-label="Default select example">
								<c:forEach items="${shareState}" var="ps">
									 <c:choose>
							            <c:when test="${ps.shrName eq share.shrName}">
							                <option value="${ps.bpShrYn}" selected="selected">${ps.shrName}</option>
							            </c:when>
							            <c:otherwise>
							                <option value="${ps.bpShrYn}">${ps.shrName}</option>
							            </c:otherwise>
							        </c:choose>
								</c:forEach>
							</select>
						 </span></li>
					<li class="d-flex w-50  align-items-center mb-2"><span class="list-tit">조회수</span> <span>${share.bpCnt }</span></li>
				</ul>
			</div>
		</div>
			<!--  글 내용  -->
				<div class="view_content" id="viewEdit">${share.bpCn }</div>
...
<!-- modify veiw -->
	<input type="hidden" name="bpId" value="${share.bpId}">
<!-- button -->
<div class="btn-box clearfix mt-4">
		<a href="${pageContext.request.contextPath }/board/share"><button class="btn btn-danger">취소</button></a>
   ...
   CKEDITOR.ClassicEditor.create(viewEdit, {
                ...
                removePlugins: [
	            	...
	            	$(".close").on("click",function(e){
	            		...
	            		$(this).parent().remove();
	            		fetch("${pageContext.request.contextPath}/board/share/file/del/"+issffId,{
							method:"POST",
							headers:{
...
// 								"Content-type":"application/json",
							...
							if(jsonObj.msg=="ok"){
								...
								window.location.href="${pageContext.request.contextPath}/board/share"
							}else{
