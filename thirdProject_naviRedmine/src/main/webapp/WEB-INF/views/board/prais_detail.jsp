...
<form id="deleteForm"
	action="${pageContext.request.contextPath }/board/prais/del/${prais.bpId }">
	...
	<div class="view_inner">
		<div class="view-info">
			<strong class="view-tit icon-tit"> <span class="icon photo"></span> 
				<em>${prais.bpTitle }</em></strong>
			<ul class="ul clearfix">

				<li><span class="list-tit">작성자</span><span>${prais.empName }</span></li>
				<li><span class="list-tit">등록일자</span><span>${prais.bpRegDtStr }</span></li>
				<li><span class="list-tit">수정자</span>
				<span><c:if test="${empty prais.bpModId}">수정내역 없음</c:if>
					<c:if test="${not empty prais.bpModId}">ADMIN</c:if>
					</span></li>
				...
				<li><span class="list-tit">조회수</span><span>${prais.bpCnt }</span></li>
			</ul>
		</div>
							...
							<div class="addfile">
								<ul class="fileList">
									<li
										...
										<a href="${pageContext.request.contextPath}/board/prais/download/${f.bpFileId}" class="file_item d-flex align-items-center">
											<c:choose>
...
<div class="btn-box clearfix mt-4">
	<a href="${pageContext.request.contextPath }/board/prais"><button class="btn btn-secondary">목록</button></a>
	<div class="right float-end">
	<c:if test="${realUser.empId eq  prais.empId}">
		<button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal">삭제</button>
		<a href="${pageContext.request.contextPath }/board/prais/mod/${prais.bpId}"><button class="btn btn-primary">수정</button></a>
		</c:if>
	</div>
</div>
<!-- button -->
...
// 	$("a[data-f-id]").on("click",function(e){
// 		let bpFileId=$(this).data("fId");
// 		fetch("${pageContext.request.contextPath}/board/prais/download/"+bpFileId,{
// 			method:"GET",
// // 			headers:{
...
// 	})
	$(deleteBtn).on("click",function(e){
		...
		fetch(deleteForm.action,{
			...
			window.location.href="${pageContext.request.contextPath}/board/prais"
		}else{
		...
		$(".commentRegBtn1").on("click",function(e){
			...
			fetch(cPath+"/board/prais/comment/reg",{
				method:"POST",
				headers:{
					"Accept":"application/json",
...
// 					 window.location.reload();
	...
	$(".delete-comment").on("click",function(e){
			...
			$("#delcomment").on("click",function(e){
				...
				fetch(cPath+"/board/prais/delcomment?bcId="+bcId,{
					method:"DELETE",
					headers:{
						"Accept":"application/json",
						[getCsrfHeader()]: getCsrfToken()
