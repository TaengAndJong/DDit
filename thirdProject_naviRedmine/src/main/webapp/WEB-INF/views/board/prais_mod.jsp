...
<form:form
	action="${pageContext.request.contextPath }/board/prais/mod/${prais.bpId }"
	id="updateForm" enctype="multipart/form-data" modelAttribute="prais">
		...
		<div class="view_inner">
			...
			<div class="view-info">
				...
				<strong class="view-tit d-flex align-items-center">
					...
					<div class="col-sm-11">
						<input type="text" name="bpTitle" class="form-control"
							placeholder="칭찬 할 사원을 입력해주세요!" value="${prais.bpTitle }">
					</div>
				</strong>
				...
				<ul class="ul clearfix">
					...
					<li><span class="list-tit">등록일</span> <span>${prais.bpRegDtStr }</span></li>
					<li><span class="list-tit">수정자</span> <span><c:if
								test="${empty prais.bpModId}">수정내역 없음</c:if> <c:if
								test="${not empty prais.bpModId}">ADMIN</c:if> </span></li>
					<li><span class="list-tit">수정일자</span> <span> <c:if
								test="${empty prais.bpModDt }">수정내역 없음</c:if> <c:if
								test="${not empty prais.bpModDt }">${prais.bpModDtStr }</c:if>
					</span></li>
					<li><span class="list-tit">조회수</span> <span>${prais.bpCnt }</span></li>
				</ul>
			</div>
			<div class="view_content" id="viewEdit">${prais.bpCn }</div>
...
<div class="btn-box clearfix mt-4">
	<a href="${pageContext.request.contextPath }/board/prais"><button class="btn btn-danger">취소</button></a>
   ...
   CKEDITOR.ClassicEditor.create(viewEdit, {
                ...
                removePlugins: [
	            	...
	            	$(".close").on("click",function(e){
	            		...
	            		$(this).parent().remove();
	            		fetch("${pageContext.request.contextPath}/board/prais/file/del/"+issffId,{
							method:"POST",
							headers:{
