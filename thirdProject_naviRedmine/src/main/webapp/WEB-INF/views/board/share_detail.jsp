...
<form id="deleteForm"
	action="${pageContext.request.contextPath }/board/share/del/${share.bpId }">
	<div class="modal info fade info" id="deleteModal" tabindex="-1"
...
<div class="view">
	<div class="view_inner">
		<div class="view-info">
			<span class="category project m0 mb-2">${share.shrName}</span> <strong
				class="view-tit icon-tit"><span class="icon present_b mb-2"></span>
				<em>${share.bpTitle}</em></strong>
			<ul class="ul clearfix">

				<li><span class="list-tit">작성자</span><span>${share.empName }</span></li>
				<li><span class="list-tit">등록일자</span><span>${share.bpRegDtStr }</span></li>
				<li><span class="list-tit">조회수</span><span>${share.bpCnt}</span></li>
				<li><span class="list-tit">수정일자</span> <span><c:if
							test="${empty share.bpModDt}">수정내역 없음</c:if> <c:if
							test="${not empty share.bpModDt}">${share.bpModDtStr }</c:if> </span></li>
							...
							<div class="addfile">
								<ul class="fileList">
									<li
										...
										<a href="${pageContext.request.contextPath}/board/share/download/${f.bpFileId}" class="file_item d-flex align-items-center"> <c:choose>
...
<div class="btn-box clearfix mt-4">
	<a href="${pageContext.request.contextPath }/board/share"><button class="btn btn-secondary">목록</button></a>
	<div class="right float-end">
		<c:if test="${realUser.empId eq  share.empId}">
			<button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal">삭제</button>
			<a href="${pageContext.request.contextPath }/board/share/mod/${share.bpId}"><button class="btn btn-primary">수정</button></a>
		</c:if>
	</div>
</div>
<!-- button -->
	...
	$(deleteBtn).on("click",function(e){
	 ...
	 $(".top-comment").on("click", function(e) {
		...
		$(".commentRegBtn1").on("click",function(e){
			...
			fetch(cPath+"/board/share/comment/reg",{
				method:"POST",
				headers:{
					"Accept":"application/json",
...
// 					 window.location.reload();
	...
	$(".delete-comment").on("click",function(e){
			...
			$("#delcomment").on("click",function(e){
				...
				fetch(cPath+"/board/share/delcomment?bcId="+bcId,{
					method:"DELETE",
					headers:{
						"Accept":"application/json",
						[getCsrfHeader()]: getCsrfToken()
