...
<form:form action="${pageContext.request.contextPath}/project/filesave/mod/${fs.fsnId }" id="updateForm">
<div class="modal fade info" id="updateModal" tabindex="-1"
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body" id="updateResult">"파일글을 수정하시겠습니까?"</div>
...
<div class="view task">
	<div class="view_inner">
		<div class="view-info">
			<strong class="view-tit d-flex align-items-center"><div class="mx-2"> <span class="category project ">${fs.fsnId }</span></div>
				<div class="col-sm-11"><input type="text" class="form-control" name="fsnTitle"
						placeholder="프로잭트 명을 입력해주세요" value="${fs.fsnTitle }"></div>
				</strong>
			<ul class="ul clearfix">
				...
				<li><span class="list-tit">등록일</span> <span class=" col-sm-6"
					aria-label="Default select example"><span>${fs.fsnRegDtStr }</span></span></li>
				<li><span class="list-tit">수정일</span>
				<span>
					<c:if test="${empty fs.fsnModDt }">수정내역 없음</c:if>
					<c:if test="${not empty fs.fsnModDt }">${fs.fsnModDtStr }</c:if>
				</span>
				</li>
				<!-- 프로젝트 상태값:
					...
					<ul class="fileList">
						<li
							class="d-flex file-row align-items-center justify-content-between mb-2">
							 <i data-id="${f.fsfId}" class="icon close"><em class="sr-only">첨부파일삭제</em></i> 
							<a href="${pageContext.request.contextPath }/project/filesave/download/${f.fsfId}" class="file_item d-flex align-items-center"><c:choose>
	 ...
	 let editor;
	   CKEDITOR.ClassicEditor.create(viewEdit, {
	            	...
	            	$(".close").on("click",function(e){
	            		...
	            		$(this).parent().remove();
	            		fetch("${pageContext.request.contextPath}/project/filesave/file/del/"+issffId,{
							method:"POST",
							headers:{
...
// 	        					"Content-type":"application/json",
	        				...
	        				console.log("111111111",jsonObj)
	        				if(jsonObj.msg='ok'){
	        					updateResult.innerHTML='파일글 수정에 성공했습니다.'
	        						window.location.pathname='${pageContext.request.contextPath}/project/filesave/${project.proId}/${fs.fsnId}'
	        				}else{
	        					updateResult.innerHTML='파일글 수정에 실패했습니다.'
	        				}
	        			}).catch(err=>{
	        				console.error(err);
	        			})
