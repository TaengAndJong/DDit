...
var fnRemoveeProperty=(event)=>{
	if(!event.target.classList.contains("delBtn")) return false;
	var result = confirm("지울거야?");
	if(!result)return fals;
	let del=event.target
	let propertyNameTag=del.nextElementSibling
	let propertyName = propertyNameTag.innerHTML
	fnCommFetch(`${baseURI}/${propertyName}`, {
			method:"delete",
			headers: {
...
document.addEventListener("click",fnRemoveeProperty);
